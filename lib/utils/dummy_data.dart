import 'package:instagram_clone_assignment/models/post_model.dart';
import 'package:instagram_clone_assignment/models/story_model.dart';

List<PostModel> dummyPosts = [

  PostModel(
    username: "sarcasticschool_",
    profileImage: "https://i.pravatar.cc/150?img=4",
    caption: "IPL pre-season fan event 🔥",
    images: [
      "https://images.unsplash.com/photo-1503023345310-bd7c1de61c7d",
      "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee",
    ],
  ),

  PostModel(
    username: "luxury_villa",
    profileImage: "https://i.pravatar.cc/150?img=7",
    caption: "Luxury meets nature 🌿",
    images: [
      "https://images.unsplash.com/photo-1600585154340-be6161a56a0c"
    ],
  ),

  PostModel(
    username: "travel_addict",
    profileImage: "https://i.pravatar.cc/150?img=10",
    caption: "Exploring the mountains 🏔️",
    images: [
      "https://images.unsplash.com/photo-1501785888041-af3ef285b470"
    ],
  ),

  PostModel(
    username: "foodielife",
    profileImage: "https://i.pravatar.cc/150?img=12",
    caption: "Best burger in town 🍔",
    images: [
      "https://images.unsplash.com/photo-1550547660-d9450f859349"
    ],
  ),

  PostModel(
    username: "tech_world",
    profileImage: "https://i.pravatar.cc/150?img=14",
    caption: "Coding late night 💻",
    images: [
      "https://images.unsplash.com/photo-1518770660439-4636190af475"
    ],
  ),

  PostModel(
    username: "nature_view",
    profileImage: "https://i.pravatar.cc/150?img=16",
    caption: "Sunset vibes 🌅",
    images: [
      "https://images.unsplash.com/photo-1491553895911-0055eca6402d"
    ],
  ),

  PostModel(
    username: "fitness_daily",
    profileImage: "https://i.pravatar.cc/150?img=18",
    caption: "Morning workout 💪",
    images: [
      "https://images.unsplash.com/photo-1517836357463-d25dfeac3438"
    ],
  ),

  PostModel(
    username: "city_explorer",
    profileImage: "https://i.pravatar.cc/150?img=20",
    caption: "City lights at night 🌃",
    images: [
      "https://images.unsplash.com/photo-1494526585095-c41746248156"
    ],
  ),

  PostModel(
    username: "coffee_addict",
    profileImage: "https://i.pravatar.cc/150?img=22",
    caption: "Morning coffee ☕",
    images: [
      "https://images.unsplash.com/photo-1509042239860-f550ce710b93"
    ],
  ),

  PostModel(
    username: "beach_vibes",
    profileImage: "https://i.pravatar.cc/150?img=24",
    caption: "Ocean breeze 🌊",
    images: [
      "https://images.unsplash.com/photo-1507525428034-b723cf961d3e"
    ],
  ),

  PostModel(
    username: "gaming_zone",
    profileImage: "https://i.pravatar.cc/150?img=26",
    caption: "Weekend gaming session 🎮",
    images: [
      "https://images.unsplash.com/photo-1511512578047-dfb367046420"
    ],
  ),

  PostModel(
    username: "art_gallery",
    profileImage: "https://i.pravatar.cc/150?img=28",
    caption: "Street art inspiration 🎨",
    images: [
      "https://images.unsplash.com/photo-1500534623283-312aade485b7"
    ],
  ),

  PostModel(
    username: "road_trip",
    profileImage: "https://i.pravatar.cc/150?img=32",
    caption: "Road trip adventures 🚗",
    images: [
      "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429"
    ],
  ),

  PostModel(
    username: "music_vibes",
    profileImage: "https://i.pravatar.cc/150?img=34",
    caption: "Late night music 🎵",
    images: [
      "https://images.unsplash.com/photo-1507838153414-b4b713384a76"
    ],
  ),

  // PostModel(
  //   username: "fashion_daily",
  //   profileImage: "https://i.pravatar.cc/150?img=36",
  //   caption: "OOTD ✨",
  //   images: [
  //     "https://images.unsplash.com/photo-1520975698519-59d5f8f1c1c1"
  //   ],
  // ),

  PostModel(
    username: "sunrise_magic",
    profileImage: "https://i.pravatar.cc/150?img=38",
    caption: "Golden sunrise 🌄",
    images: [
      "https://images.unsplash.com/photo-1502082553048-f009c37129b9"
    ],
  ),

  PostModel(
    username: "tech_setup",
    profileImage: "https://i.pravatar.cc/150?img=40",
    caption: "My workstation 💻",
    images: [
      "https://images.unsplash.com/photo-1498050108023-c5249f4df085"
    ],
  ),

  PostModel(
    username: "food_diaries",
    profileImage: "https://i.pravatar.cc/150?img=42",
    caption: "Dessert time 🍰",
    images: [
      "https://images.unsplash.com/photo-1505253216365-8c70a3d9d7b6"
    ],
  ),

  PostModel(
    username: "mountain_hiker",
    profileImage: "https://i.pravatar.cc/150?img=44",
    caption: "Hiking day 🥾",
    images: [
      "https://images.unsplash.com/photo-1500530855697-b586d89ba3ee"
    ],
  ),

];


List<StoryModel> dummyStories = [

  StoryModel(
    username: "aditya",
    imageUrl: "https://i.pravatar.cc/150?img=1",
  ),

  StoryModel(
    username: "rahul",
    imageUrl: "https://i.pravatar.cc/150?img=2",
  ),

  StoryModel(
    username: "sarthak",
    imageUrl: "https://i.pravatar.cc/150?img=3",
  ),

  StoryModel(
    username: "rohit",
    imageUrl: "https://i.pravatar.cc/150?img=4",
  ),

  StoryModel(
    username: "neha",
    imageUrl: "https://i.pravatar.cc/150?img=5",
  ),

  StoryModel(
    username: "vikas",
    imageUrl: "https://i.pravatar.cc/150?img=6",
  ),

  StoryModel(
    username: "pallavi",
    imageUrl: "https://i.pravatar.cc/150?img=7",
  ),

  StoryModel(
    username: "amit",
    imageUrl: "https://i.pravatar.cc/150?img=8",
  ),

  StoryModel(
    username: "sneha",
    imageUrl: "https://i.pravatar.cc/150?img=9",
  ),

  StoryModel(
    username: "karan",
    imageUrl: "https://i.pravatar.cc/150?img=10",
  ),

];